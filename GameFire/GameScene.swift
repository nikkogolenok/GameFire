...
//  GameFire
//
//  Created by Никита Коголенок on 31.07.21.
//
...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMove(to view: SKView) {
        ...
        motionManager.startAccelerometerUpdates(to: OperationQueue.current!) { (data: CMAccelerometerData?, error: Error?) in
            if let accelerometerData = data {
                let accelerometion = accelerometerData.acceleration
                self.xAccelerate = CGFloat(accelerometion.x) * 0.75 + self.xAccelerate * 0.25
            }
        }
    }
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        if (alienBody.categoryBitMask & alienCategory) != 0 &&
            ...
            collisionElement(bulletNode: bulletBody.node as! SKSpriteNode, alienNode: alienBody.node as! SKSpriteNode)
        }
    }
